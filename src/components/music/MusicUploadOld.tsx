...
import { Upload, Music, X, CheckCircle, AlertCircle, Image, Edit3, Save, Eye } from 'lucide-react';
...
const GENRES = [
  ...
  'Reggae', 'Blues', 'Folk', 'Punk', 'Metal', 'Ambient', 'House', 'Techno',
  'Dubstep', 'Trap', 'Lo-fi', 'Synthwave', 'Indie', 'Alternative'
];

const MOODS = [
  'Energetic', 'Chill', 'Melancholic', 'Happy', 'Aggressive', 'Peaceful',
  'Dark', 'Uplifting', 'Romantic', 'Mysterious', 'Dreamy', 'Intense'
];
...
export function MusicUpload({ 
  ...
  const allMetadataComplete = metadataFiles.every(f => f.metadata?.title.trim());
        ...
        <GlassCard className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-lg font-semibold text-white">
              {currentStep === 'complete' ? 'Upload Complete' : `Processing ${files.length} files`}
            </h4>
          ...
          <div className="space-y-4">
            {files.map((file) => (
              <div key={file.id} className="bg-black/20 rounded-lg overflow-hidden">
                {/* File Header */}
                <div className="flex items-center space-x-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {file.error ? (
                      ...
                      <p className="text-xs text-blue-400">Ready for metadata</p>
                ...
                {/* Metadata Editor */}
                {file.status === 'metadata' && editingFile === file.id && file.metadata && (
                  <div className="border-t border-gray-700 p-4 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="w-20 h-20 bg-gradient-to-br from-fuchsia-500 to-teal-500 rounded-lg overflow-hidden flex items-center justify-center">
                            {file.metadata.coverArtPreview ? (
                              <img src={file.metadata.coverArtPreview} alt="Cover" className="w-full h-full object-cover" />
                            ) : (
                          ...
                          <div>
                            <input
                              ...
                              type="file"
                              accept="image/*"
                              onChange={(e) => handleCoverArtSelect(file.id, e)}
                              className="hidden"
                            />
                            <Button
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => updateMetadata(file.id, { mood: e.target.value })}
                          className="w-full px-3 py-2 bg-gray-800/50 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-fuchsia-500 transition-colors"
                        >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateMetadata(file.id, { bpm: parseInt(e.target.value) || undefined })}
                          className="w-full px-3 py-2 bg-gray-800/50 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-fuchsia-500 transition-colors"
                          placeholder="e.g. 120"
                          min="60"
                          max="200"
                    ...
                    <div className="flex items-center space-x-3">
                      <input
                        ...
                        onChange={(e) => updateMetadata(file.id, { isExclusive: e.target.checked })}
                        className="rounded border-gray-600 text-fuchsia-500 focus:ring-fuchsia-500"
                      />
                      <label htmlFor={`exclusive-${file.id}`} className="text-sm text-gray-300">
