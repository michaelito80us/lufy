...
export default function MusicAdminDashboard() {
  ...
  const toggleTrackExclusivity = async (
    trackId: string,
    isExclusive: boolean
  ) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          `Track ${!isExclusive ? 'made exclusive' : 'made public'}`
        )
      } else {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">
              Manage your music library and fan engagement
            </p>
          </div>
          <Button
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            toggleTrackExclusivity(track.id, track.isExclusive)
                          }
              ...
              <CardContent>
                <div className="space-y-4">
                  {subscribers.map((subscriber) => (
                    <div
                      key={subscriber.id}
                      className="flex items-center space-x-4 p-4 rounded-lg bg-gray-700/30"
                    >
                      {subscriber.user.image ? (
                        <img
                          src={subscriber.user.image}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-4 text-sm text-gray-400 mt-1">
                          <span>
                            Started: {formatDate(subscriber.startDate)}
                          </span>
                          <span>Expires: {formatDate(subscriber.endDate)}</span>
                          <span>${subscriber.price}/month</span>
                        </div>
                      </div>
